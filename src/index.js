...
async function checkStatus(octokit, context) {
  ...
  return {
    hasChecks: allChecks.length > 0,
    allCompleted: allChecks.every(check => 
      check.status === 'completed' || check.state === 'success'
    ),
    allPassed: allChecks.every(check => 
      check.conclusion === 'success' || check.state === 'success'
    ),
    pending: allChecks.filter(check => 
      check.status === 'in_progress' || check.status === 'queued'
    ).map(check => check.name),
    failed: allChecks.filter(check =>
  ...
  try {
    ...
    while (true) {
      ...
      try {
        ...
        } else if (status.allCompleted && status.allPassed && !hasNotified) {
